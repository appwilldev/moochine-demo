...
apt-get install build-essential daemontools bzip2 zip libreadline-dev libncurses5-dev libpcre3-dev libssl-dev perl libyaml-dev libmagickcore-dev libmagickwand-dev libcloog-ppl0 libgeoip-dev git htop iotop tmux rcconf rlwrap strace vim curl python-dev python-setuptools
...
rm -rf postgresql-9.3.4.tar.gz postgresql-9.3.4
wget http://ftp.postgresql.org/pub/source/v9.3.4/postgresql-9.3.4.tar.gz
tar vfxz postgresql-9.3.4.tar.gz
cd postgresql-9.3.4
./configure
make -j6
make install
...
rm -rf redis-2.8.13.tar.gz redis-2.8.13
...
rm -rf ngx_openresty-1.7.2.1.tar.gz ngx_openresty-1.7.2.1
wget http://agentzh.org/misc/nginx/ngx_openresty-1.7.2.1.tar.gz
